...
try:
    ...
    root_err = tk.Tk()
    root_err.withdraw()
    msgbox.showinfo("Error", "IMU not connected")
    quit()
...
def stop_loop():
    if opt_button["text"] == "Open menu":
        msgbox.showinfo("Error", "Please choose an option")
    else:
        global running
        running = False
...
def plot_data():
    if opt_button["text"] == "Open menu":
        ...
        msgbox.showinfo("Error", "Please start and stop the option before plotting")
    else:
        if plt.get_fignums():
            plt.close('all')
...
def open_menu():
    ...
    menu.add_command(label="Option 1: Acceleration, Angular Rate & Magnetometer Vector",
                     command=lambda: option_selected("Option 1", option1))
    menu.add_command(label="Option 2: Temperatures", command=lambda: option_selected("Option 2", option2))
    menu.add_command(label="Option 3: Raw Accelerometer and Angular Rate Sensor Outputs",
                     command=lambda: option_selected("Option 3", option3))
    menu.add_command(label="Option 4: Gyro Stabilized Acceleration, Angular Rate & Magnetometer Vector",
                     command=lambda: option_selected("Option 4", option4))
    menu.add_command(label="Option 5: DeltaAngle & DeltaVelocity & Magnetometer Vectors",
                     command=lambda: option_selected("Option 5", option5))
    menu.add_command(label="Option 6: Euler Angles and Angular Rates",
                     command=lambda: option_selected("Option 6", option6))
    menu.add_command(label="Option 7: Orientation Matrix",
                     command=lambda: option_selected("Option 7", option7))
    menu.add_command(label="Option 8: Attitude Update Matrix",
                     command=lambda: option_selected("Option 8", option8))
...
def last_option():
    if len(last_choice) >= 2:
        if last_choice[-1] != last_choice[-2]:
            print("option changed")
            initial_cond()


def option_selected(text, func=None):
...
root = tk.Tk()
root.title("3DM-GX2 IMU")
root.geometry("450x270")

photo = tk.PhotoImage(file='jobs.png')
root.wm_iconphoto(False, photo)

image = PhotoImage(file="imu.png") # Add this line to load the image
label = tk.Label(root, image=image) # Add this line to display the image
label.pack()
...
start_button = tk.Button(button_frame, text="Start", command=start_loop, font=("Helvetica", 10), width=10, height=2)
start_button.pack(side="left")

stop_button = tk.Button(button_frame, text="Stop", command=stop_loop, font=("Helvetica", 10), width=10, height=2)
stop_button.pack(side="left")

opt_button = tk.Button(button_frame, text="Open Menu", command=open_menu, font=("Helvetica", 10), width=10, height=2)
opt_button.pack(side="left")

plot_button = tk.Button(button_frame, text="Plot", command=plot_data, font=("Helvetica", 10), width=10, height=2)
plot_button.pack(side="left")

plot_button = tk.Button(button_frame, text="Reset", command=reset, font=("Helvetica", 10), width=10, height=2)
plot_button.pack(side="left")
